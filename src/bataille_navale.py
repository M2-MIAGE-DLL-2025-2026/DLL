...
def place_bateaux(grille: List[List[str]], nb_bateau_a_place: int) -> List[Tuple[int, int]]:
    """Place aléatoirement `nb_bateau_a_place` bateaux sur la grille.
    ...
    size = len(grille)
    while len(bateaux) < nb_bateau_a_place:
...
def choix_utilisateur() -> Tuple[int, int]:
    ...
    Vérifie que la saisie est valide ; la fonction retourne deux
